// ai-excuse.js — usa SEMPRE i 3 modelli base per ogni kind/SKU e genera 3 varianti.
// API compatibile: accetta {kind, need, style, locale, maxLen} o {sku}.
...
function vary(base, i, max) {
  if (i === 0) return base.slice(0, max); // prima = modello puro
  const twist = (i === 1) ? TWISTS_A[i % TWISTS_A.length] : TWISTS_B[i % TWISTS_B.length];
  let out = base.trim();
  if (!/[.!?]$/.test(out)) out += '.';
  out += ' ' + twist;
...
exports.handler = async (event) => {
  ...
  const kind = kindRaw || mapSkuToKind(sku) || 'base';
  const maxLen = Math.max(140, Math.min(360, Number(body.maxLen || 320)));

  const bank = BASE[kind] || BASE.base;
  const variants = [0, 1, 2].map(i => ({ whatsapp_text: vary(bank[i], i, maxLen) }));
