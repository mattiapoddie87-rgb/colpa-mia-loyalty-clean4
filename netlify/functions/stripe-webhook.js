...
// - Genera 3 scuse via ai-excuse (in base allo SKU/kind)
// - Invia Email (Resend) con mittente @colpamia.com
// - Invia WhatsApp (Twilio)
// - Aggiorna metadata sul PaymentIntent
// - Ritorna JSON con stati/diagnosi
...
const TW_FROM_WA = (process.env.TWILIO_FROM_WA     || '').trim(); // es. "whatsapp:+39XXXXXXXXX" o sandbox
...
// mappa SKU -> kind (allinea ai tuoi pacchetti)
function skuToKind(sku) {
  const s = String(sku || '').toUpperCase();
  if (s === 'RIUNIONE') return 'riunione';
  if (s === 'TRAFFICO') return 'traffico';
  if (s === 'SCUSA_TRIPLA' || s.includes('TRIPLA')) return 'tripla';
  if (s === 'SCUSA_DELUXE' || s.includes('DELUXE')) return 'deluxe';
  if (s === 'CONS_KO' || s === 'CONN_KO' || s.includes('CONNESSIONE')) return 'connessione';
  return 'base';
}

// normalizza telefono in E.164
...
// estrae telefono dal campo ufficiale o dai custom_fields (key: phone/whatsapp/wa)
function getWhatsAppNumber(session) {
  const cand = [];
  const cPhone = pick(session, 'customer_details.phone', '');
  if (cPhone) cand.push(cPhone);
...
// minuti dai line items (rules > price.metadata.minutes > product.metadata.minutes)
...
// 3 scuse dal tuo endpoint (con fallback locale)
...
// Resend Email (mittente @colpamia.com)
async function sendEmail(to, subject, html) {
  ...
  try {
    const r = await fetchFn('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'COLPA MIA <no-reply@colpamia.com>', // dominio da verificare su Resend
        to: [to],
        subject,
        html,
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const email  = String(pick(session, 'customer_details.email', '') || '').toLowerCase();
    ...
    // contesto da custom_fields.key === 'need'
    let need = '';
    try {
    ...
    let wa = { ok: false, reason: 'skip' };
    if (phoneE164) wa = await sendWhatsApp(phoneE164, bodyWA);
    const waStatus = wa.ok ? 'sent' : `fail:${wa.reason || 'unknown'}`;
