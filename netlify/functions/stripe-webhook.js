...
// ===== AI helpers: garantiamo 3 varianti uniche =====
...
function localVariations(seed){
  const b = String(seed||'Imprevisto ora, sto riorganizzando. Ti aggiorno entro sera.').trim();
...
async function threeExcuses(need, persona){
  const styles=['neutro','executive','soft','tecnico'];
  const uniq=new Map();
  for(const s of styles){
    const list=await askAI({need, persona, style:s});
    for(const t of list){ const k=t.toLowerCase(); if(t && !uniq.has(k)) uniq.set(k,t); if(uniq.size>=3) break; }
    if(uniq.size>=3) break;
  }
  if(uniq.size===0) localVariations().forEach(t=>uniq.set(t.toLowerCase(),t));
  if(uniq.size<3){
    const base=[...uniq.values()][0] || need || '';
    for(const t of localVariations(base)){ const k=t.toLowerCase(); if(!uniq.has(k)) uniq.set(k,t); if(uniq.size>=3) break; }
  }
  return [...uniq.values()].slice(0,3);
}

// ===== minuti dai line items (rules o metadata) =====
...
async function sendWhatsApp(toRaw, body, piId){
  ...
  try{ await twilio.messages.create({ from:twilioFromWa, to:asWhatsApp(toRaw), body }); return {ok:true}; }
  catch(e){ try{ if(piId) await stripe.paymentIntents.update(piId,{metadata:{colpamiaWaError:String(e?.message||'wa_error')}});}catch{}; return {ok:false}; }
}
...
exports.handler = async (event)=>{
  ...
  let variants = await threeExcuses(need, persona);
  if(variants.length<3){ // fail-safe
    variants = [...new Set([...variants, ...localVariations(need)])].slice(0,3);
  }
  ...
  // upsert Customer by email per wallet persistente
  ...
  if(!customerId && email){
    try{
      ...
      if(found?.data?.[0]) customerId = found.data[0].id;
        ...
        const created = await stripe.customers.create({ email, name: pick(session,'customer_details.name','')||undefined });
  ...
  return j(200,{ok:true,minutes,email,waStatus,excuses:variants.length});
};
