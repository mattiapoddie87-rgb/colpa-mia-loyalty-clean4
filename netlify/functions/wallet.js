...
async function minutesFrom(session){
  const R = rules();
  const li = await stripe.checkout.sessions.listLineItems(session.id,{limit:100,expand:['data.price.product']}).catch(()=>({data:[]}));
  let sum=0;
  for(const it of (li.data||[])){
    const qty=it?.quantity||1, priceId=it?.price?.id;
    if(priceId && R[priceId]){ sum += Number(R[priceId].minutes||0)*qty; continue; }
    const m1=Number(pick(it,'price.metadata.minutes',0))||0;
    const m2=Number(pick(it,'price.product.metadata.minutes',0))||0;
    sum += (m1||m2)*qty;
  }
  return sum;
}
...
exports.handler = async (event)=>{
  ...
  try{
    // trova customer
    const srch = await stripe.customers.search({ query:`email:"${email}"`, limit:1 });
    const customer = srch?.data?.[0];
