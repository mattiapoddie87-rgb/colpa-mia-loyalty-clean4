...
// Recupera la sessione Checkout, valida, ricava email e accredita minuti (idempotente via PI metadata).
...
// Mappatura minuti: ENV JSON {"price_XXX":5,"price_YYY":15} oppure metadata.minutes su Price/Product
...
exports.handler = async (event) => {
  try {
    ...
    // Email autoritativa dalla Sessione, fallback solo se assente
    const emailFromSession = (s.customer_details && s.customer_details.email) || s.customer_email || null;
    const email = (emailFromSession && String(emailFromSession).toLowerCase())
